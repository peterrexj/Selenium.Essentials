...
$sapiExt = [IO.Path]::Combine($PSScriptRoot, '..\Output\TestAny.Essentials.Api.Extensions' + $packageVersion)
...
Get-ChildItem -Path $sapiExt -ErrorAction Stop
...
dotnet nuget push $sapiExt --api-key $apiKey --source https://api.nuget.org/v3/index.json
